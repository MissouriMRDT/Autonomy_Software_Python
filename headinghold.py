...
def headingHold(goal, quaternion_ref, drive, speed):
    ...
    if abs(goal - quaternion_ref.trueHeading) < 5 or abs(quaternion_ref.roll) > 15:
        print("Going straight for goal! Roll: %f" %(quaternion_ref.roll))
        drive.move(speed, 0)
    else:
        correction = pid.update(goal, quaternion_ref.trueHeading)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if heading != prevheading:
            ...
            prevheading = heading
        print("\tGoal: %f \tHeading: %f" % (goal, quaternion.trueHeading))
        time.sleep(0.1)
