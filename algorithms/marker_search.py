...
def calculate_next_coordinate(start, former_goal):
    ...
    theta += (
        constants.DELTA_THETA
    )  # We should definitely decrease theta from math.pi/2 to pi/4, pi/6 or even pi/8
    ...
    return constants.Coordinate(
        start.lat + diff_lat / 1000, start.lon + diff_lon / 1000
    )
