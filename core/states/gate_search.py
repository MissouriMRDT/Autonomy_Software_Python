...
class GateSearch(RoverState):
    ...
    async def run(self) -> RoverState:
        ...
        tag_latitude, tag_longitude = geomath.reverse_haversine(
            bearing, distance, current[0], current[1]
        )  # *** Reverse haversince is in km ***

        # Compute lat and long to approach exactly 4m from tag, then append to waypoint deque *** THIS NEEDS TO BE DEGRADATED
        tag_approach_lat, tag_approach_long = geomath.reverse_haversine(
            bearing, distance - 0.004, current[0], current[1]
        )
        ...
        self.logger.info(
            f"Added Position Waypoint to Front of Queue: lat ({tag_approach_lat}), lon ({tag_approach_long})"
        )  # Consider updating goal coordinate

        # Compute waypoints that approximate circle and append to list
        circle_points = [waypoint]
        circle_points += self.compute_circle_waypoints(
            tag_latitude, tag_longitude, 4, 8
        )  # 2nd and third parameters are radius (in m) and increments (number of waypoints)
        self.logger.info(
            f"Added position waypoints that go in circle around AR tag: lat ({tag_latitude}), lon ({tag_longitude})"
        )
    ...
    def compute_circle_waypoints(self, tag_lat, tag_long, circle_radius, increments):
        ...
        *** STILL A WORK IN PROGRESS AND UNTESTED ***
        ...
        for i in range(increments):
            ...
            new_angle = bearing + angle_diff * (
                i + 1
            )  # *** Need to verify whether reverse_haversine can handle angles outside 0-360 range ***
